...
# ## **Multimodal-Image-Registration-Affine-Model**
...
# fixed image
original_I = io.imread("images/b1.png").astype(np.float32) 
# moving image
original_J = io.imread("images/b2.png").astype(np.float32) 
...
if np.ndim(I) == 3:
    ...
    print("invalid rank for an image")
...
def multi_resolution_loss():
    ...
    for s in np.arange(L-1,-1,-1):
        if nChannel>1:
            ...
            Jw_ = AffineTransform(J_lst[s].unsqueeze(0).unsqueeze(0), homography_net(s), xy_lst[s][:,:,0], 
                                  xy_lst[s][:,:,1]).squeeze()
...
for s in range(L):
    I_ = torch.tensor(cv2.normalize(pyramid_I[s].astype(np.float32), None, alpha=0, beta=1,
                                    norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)).to(device)
    J_ = torch.tensor(cv2.normalize(pyramid_J[s].astype(np.float32), None, alpha=0, beta=1,
                                    norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)).to(device)
