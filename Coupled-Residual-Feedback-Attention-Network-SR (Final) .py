...
def DCFRAN():
    inputs = keras.Input(shape=(128, 128, 3))  # Input shape for RGB images
    ...
    # Attention mechanism
    attention = Conv2D(3, (1, 1), padding='same')(x)
    ...
    # Coupled feedback
    coupled_feedback = x * attention
    coupled_feedback = Conv2D(3, (3, 3), padding='same')(coupled_feedback)
    x = Add()([x, coupled_feedback])
    outputs = layers.Conv2D(filters=3, kernel_size=3, strides=1, padding="same", activation="sigmoid")(x)  
...
def residual_attention_block(x):
    ...
    # Attention mechanism
    attention = layers.Conv2D(filters=1, kernel_size=1, strides=1, padding="same", activation="sigmoid")(x)
    x = layers.Multiply()([attention, x])
...
# Create model
#model = DCF_ResAtt(input_shape)
# Create  DCFGRAN model
model = DCFRAN()
