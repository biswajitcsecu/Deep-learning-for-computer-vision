...
from tensorflow.keras.layers import Reshape, Conv2DTranspose, Dropout, Multiply, Lambda
...
# Define the residual dense block
def residual_dense_block(input_tensor):
    x = layers.Conv2D(64, kernel_size=3, padding='same', activation='relu')(input_tensor)
    x = layers.Concatenate()([input_tensor, x])
    return x

# Define the guided residual dense network
def guided_resnet(input_shape):
    ...
    x = layers.Conv2D(3, kernel_size=3, padding='same', activation='relu')(x)
    
    # Reconstruction block
    x = layers.Add()([x, inputs])
    outputs = layers.Conv2D(3, kernel_size=3, padding='same', activation='relu')(x)
...
input_shape=[H,W,CH]
# Create the model
model = guided_resnet(input_shape)
...
x = [i for i in  tqdm(range(nrange))]
plt.plot(x,history.history['ssim_metric_1'])
