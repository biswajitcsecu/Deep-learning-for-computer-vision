...
# ## **Vocal Denoising  using DCNN Framework**
...
train_files=['Vocal/train/images/'+filename for filename in os.listdir('Vocal/train/images/')]
test_files=['Vocal/test/images/'+filename for filename in os.listdir('Vocal/test/images/')]
...
for i in range(10):
    ...
    fig.suptitle('Noisy Images',fontsize=20,fontweight='bold')
fig.align_labels()     
plt.tight_layout()    
plt.show()
...
for i in range(10):
    ...
    axs[i].get_xaxis().set_visible(False)
    axs[i].get_yaxis().set_visible(False)
    fig.suptitle('Ground Truth Images',fontsize=20,fontweight='bold')
fig.align_labels()     
plt.tight_layout()     
plt.show()
...
#Model
def CNN(inputs):
    x=Conv2D(64, (3,3), dilation_rate=1,padding='same',activation='relu')(inputs)
    x=Conv2D(64, (3,3), dilation_rate=2,padding='same',activation='relu')(x)
    
    y=Conv2D(64, (3,3), dilation_rate=4,padding='same',activation='relu')(inputs)
...
def RPDNET():
    inputs = Input((H, W, CH),name='input')
    feat_extraction =Conv2D(64, (3,3),padding='same')(inputs)
...
history=rpdnet.fit( train_dataset,shuffle=True,epochs=nepochs,
                   ...
                   max_queue_size=16,workers=4,verbose=1,
...
axs[2].title.set_text('Denoised Image')
axs[3].imshow(residual_image)
axs[3].title.set_text('Residual Image')
